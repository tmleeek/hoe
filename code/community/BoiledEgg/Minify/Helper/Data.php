<?php
/**
 * Class BoiledEgg_Minify_Helper_Data
 */
class BoiledEgg_Minify_Helper_Data extends Mage_Core_Helper_Data {

	const XML_PATH_JS_MINIFY_METHOD = 'dev/js/minify_method';
	const XML_PATH_JS_MINIFY = 'dev/js/minify_files';
	const XML_PATH_CSS_MINIFY = 'dev/css/minify_files';
		...
		$beforeMergeCallback = null, $extensionsFilter = array())
	{
		try {
			...
			if ($shouldMerge) {
				...
				$jsMin = Mage::getModel('minify/JSMin');
				$cssMin = Mage::getModel('minify/CSSMin');

				if (Mage::getStoreConfig(self::XML_PATH_JS_MINIFY_METHOD) === 'jsminplus') {
					$jsMin = Mage::getModel('minify/JSMinPlus');
				}

				if ($fileExt === 'js' && Mage::getStoreConfigFlag(self::XML_PATH_JS_MINIFY)) {
					$data = $jsMin->minify($data);
				}

				if ($fileExt === 'css' && Mage::getStoreConfigFlag(self::XML_PATH_CSS_MINIFY)) {
					$data = $cssMin->run($data);
				}
