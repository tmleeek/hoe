...
class Klevu_Search_Model_Order_Sync extends Klevu_Search_Model_Sync {
            ...
            if ($item->getId()) {
                if ($this->isEnabled($item->getStoreId())) {
                    ...
                    if ($result === true) {
                        ...
                        $this->log(Zend_Log::ERR, sprintf("Failed to sync order item %d: %s", $item_id, $result));
                        $errors++;
                    }
                } else {
    ...
    protected function sync($item) {
        ...
        $response = Mage::getModel("klevu_search/api_action_producttracking")->execute(array(
