...
class Webtex_CustomerGroupsPrice_Model_Observer
{
    ...
    /**
     * Proper way to add render class on specific attributte without overwrite the class
    ...
    public function prepareForm(Varien_Event_Observer $observer)
    {
        ...
        $customerGroupsPrice = $form->getElement('customer_groups_price');
        
        if ($customerGroupsPrice) {
            $customerGroupsPrice->setRenderer(
                Mage::app()
                    ->getLayout()
                    ->createBlock('customergroupsprice/adminhtml_catalog_product_edit_tab_price_customergroups')
            );
        }
