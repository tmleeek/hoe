        ...
        if ($requestInfo['giftwrap_add'] && $requestInfo['giftwrap_add'] == 'new') {
            if ($requestInfo['giftbox_paper']) {
                if ($productQty > $maxQtyPerBox) {
                    Mage::getSingleton('checkout/session')->addNotice(Mage::helper('giftwrap')->__('There are too many items in one gift box. The maximum number allowed is (%s). Automatically added new gift box(es) to cart.', $maxQtyPerBox));
