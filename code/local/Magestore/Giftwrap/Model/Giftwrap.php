        ...
        if ($session->getData('not_save_quote')) {
            ...
            foreach ($not_save_quote as $giftboxId) {
                if ($giftboxId) {
                     ...
                     * a foreign key constraint fails (`hoe_magento`.`giftwrap_selection`, CONSTRAINT `giftwrap_selection_ibfk_1` 
                     * FOREIGN KEY (`style_id`) REFERENCES `giftwrap` (`giftwrap_id`) ON DELETE CASCADE ON UPDATE CASCADE)
                     */
                    try {
                    ...
                    /* prevent crash -SQLSTATE[23000]: Integrity constraint violation: 
                     * 1452 Cannot add or update a child row: a foreign key constraint fails (`hoe_magento`.`giftwrap_selection`, CONSTRAINT `giftwrap_selection_ibfk_1` 
                     * FOREIGN KEY (`style_id`) REFERENCES `giftwrap` (`giftwrap_id`) ON DELETE CASCADE ON UPDATE CASCADE)
                     */
                }
            }
