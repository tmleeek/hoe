            ...
            if ($wraptype == 1) {
                ...
                foreach ($wrapall as $itemId) {
                    ...
                    if (floatval($data['qty_wrap_' . $itemId]) > floatval(Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id']))) {
                        Mage::getSingleton('core/session')->addError( Mage::helper('giftwrap')->__('There are too many items in one gift box. The maximum number allowed is (%s). Automatically added new gift box(es) to cart.', Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id'])) );
                        $this->_redirect('giftwrap/multishipping/addgiftwrap',array('address_id'=>$address_id));
                        return;
                    }
                ...
                if ($number_items > $maxItems) {
                    Mage::getSingleton('core/session')->addError( Mage::helper('giftwrap')->__('There are too many items in one gift box. The maximum number allowed is (%s). Automatically added new gift box(es) to cart.', $maxItems) );
