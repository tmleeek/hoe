...
class Magestore_Giftwrap_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function savegiftboxAction() {
        ...
        if ((!isset($data['giftbox_paper'])) || (!$data['giftbox_paper'])) {
            $error = Mage::helper('giftwrap')->__("Cannot save giftbox . Please select paper !");
        ...
        if ((!isset($data['wrap_type'])) || (!$data['wrap_type'])) {
            $error = Mage::helper('giftwrap')->__("Cannot save giftbox . Please select wrap type !");
            Mage::getSingleton('checkout/session')->addError($error);
            $this->_redirect('checkout/cart/index');
            return;
        }
        ...
        if ($wraptype) {
            ...
            if ($wraptype == 1) {
				...
				Mage::getSingleton('checkout/session')->addSuccess(Mage::helper('giftwrap')->__("Gift wrap was saved successfully."));
            }
            if ($wraptype == 2) {
                //Zend_Debug::dump($data);die();
                if ((!isset($data['wrap'])) || (!is_array($data['wrap']))) {
                    $error = Mage::helper('giftwrap')->__("Cannot save giftbox . Please select item !");
                    Mage::getSingleton('checkout/session')->addError($error);
                    $this->_redirect('checkout/cart/index');
                    return;
                }
                ...
                foreach ($wrapall as $itemId) {
                    ...
                    if (floatval($data['qty_wrap_' . $itemId]) > floatval(Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id']))) {
                        Mage::getSingleton('checkout/session')->addError( Mage::helper('giftwrap')->__('There are too many items in one gift box. The maximum number allowed is (%s). Automatically added new gift box(es) to cart.', Mage::helper('giftwrap')->getNumberOfItemsCanWraped($item, $data['giftbox_id'])) );
...
//                    }
        ...
        $maxItems = Mage::getStoreConfig('giftwrap/calculation/maximum_items_wrapall');
        if ($number > $maxItems) {
            Mage::getSingleton('checkout/session')->addError(Mage::helper('giftwrap')->__('There are too many items in one gift box. The maximum number allowed is (%s). Automatically added new gift box(es) to cart.', $maxItems));
            $this->_redirect('checkout/cart');
            return;
        } else {
            $item = trim($item, ',');
    ...
    public function deletegiftboxAction() {
        ...
        try {
            ...
            //remove giftbox
            $giftbox->delete();
            Mage::getSingleton('checkout/session')->addSuccess(Mage::helper('giftwrap')->__('Deleted gift wrap successfully.'));
        } catch (Exception $e) {
